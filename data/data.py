...
# generator handling a SR _Program object instead of an equations.ODE object
class DataGenerator_p:
    def __init__(self, f, dim_x, T, freq, n_sample, noise_sigma, init_low=0., init_high=1., return_list=False):
        self.f = f
        self.T = T
        self.freq = freq
        self.noise_sigma = noise_sigma
        self.solver = equations.ODESolver_p(f, T, freq, return_list=return_list)
        self.return_list = return_list

        if isinstance(init_high, float) or isinstance(init_high, int):
            self.init_cond = np.random.uniform(init_low, init_high, (n_sample, dim_x))
        else:
            # list of numbers
            assert len(init_high) == dim_x
            tmp = list()
            for high, low in zip(init_high, init_low):
                tmp.append(np.random.uniform(low, high, (n_sample, 1)))
            self.init_cond = np.concatenate(tmp, axis=-1)
